...
\center
```{r out.width='70%'}
ic.m <- function(x,sigma2,conf.level=0.95){ # Função que calula o intervalo de confiança
  n<-length(x)
  xbarra<-mean(x)
...
hist(Sn,breaks = 20, probability = T, ylab = "Densidade de Frequência", xlim=c(100,300), 
     xlab=expression(S[n]), main=expression(paste("Histograma de ", S[n])),col="Grey")
...
ic.m(Sn,var(Sn),0.683)[2] - Sn.bar # C1
ic.m(Sn,var(Sn),0.954) [2] - Sn.bar # C2
ic.m(Sn,var(Sn),0.997)[2] - Sn.bar # C3
```
